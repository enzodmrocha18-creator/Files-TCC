...
##   4. Respostas a impulso ........... Secao 3.4.1 / Tabela 4.1
...
# grafico de uma resposta acumulada do IPCA, com banda de confianca
...
if (!file.exists("painel_var.csv"))
  stop("Nao encontrei painel_var.csv. Rode o 01_dados.R antes deste script.")

dados <- read.csv("painel_var.csv")
...
pdf("graficos/02_series.pdf", width = 11, height = 6.5)
par(mar = c(4, 5, 4, 2))
for (j in 1:ncol(y)) {
...
pdf("graficos/03_cusum.pdf", width = 10, height = 6)
par(mar = c(4, 5, 4, 2))
...
pdf("graficos/04_respostas.pdf", width = 10, height = 6)
par(mar = c(5, 5, 4, 2))
...
pdf("graficos/05_respostas_completas.pdf", width = 10, height = 6)
...
linha <- function(objeto, impulso, escala, nome) {
  ...
  data.frame(choque = nome, horizonte = c(6, 12, 24),
             resposta = round(objeto$irf[[impulso]][h, "ipca"] / escala, 3),
             ic_inf   = round(objeto$Lower[[impulso]][h, "ipca"] / escala, 3),
             ic_sup   = round(objeto$Upper[[impulso]][h, "ipca"] / escala, 3))
}

tab_irf <- rbind(
  linha(irf_comm, "comm",   esc_comm, "Commodities (1%)"),
  linha(irf_pimp, "pimp",   esc_pimp, "P. importacao (1%)"),
  linha(irf_camb, "cambio", esc_camb, "Cambio (1%)"))

cat("\n=== TABELA 4.1: RESPOSTA ACUMULADA DO IPCA (%) ===\n")
print(tab_irf, row.names = FALSE)
write.csv(tab_irf, "tab4_1_irf.csv", row.names = FALSE)
...
write.csv(tab_cambio, "tab4_2_cambio.csv", row.names = FALSE)
...
pdf("graficos/06_decomposicao_variancia.pdf", width = 10, height = 7)
...
print(tab_fevd)
write.csv(tab_fevd, "tab4_3_fevd.csv")
...
write.csv(tab_sub, "tab4_4_subamostras.csv", row.names = FALSE)
...
write.csv(tab_assim, "tab4_5_assimetria.csv", row.names = FALSE)

cat("\nTabelas gravadas na pasta de trabalho.\n")
cat("Graficos gravados em graficos/:\n")
